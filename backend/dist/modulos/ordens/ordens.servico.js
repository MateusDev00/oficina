...
class OrdensServico {
    ...
    `;
        ...
        const result = await (0, base_de_dados_1.query)(sql, params);
        res.json(result.rows);
    }
    async obterPorId(req, res) {
        const { id } = req.params;
        const result = await (0, base_de_dados_1.query)(`SELECT o.*, 
    ...
    async criar(req, res) {
        ...
        const cliente = await (0, base_de_dados_1.getCliente)();
        try {
            await cliente.query('BEGIN');
            const ordemResult = await cliente.query(`INSERT INTO ordens_servico (cliente_id, veiculo_id, descricao, data_agendada, prioridade, estado)
         VALUES ($1, $2, $3, $4, $5, 'pendente')
         RETURNING *`, [cliente_id, veiculo_id, descricao, data_agendada || null, prioridade || 'media']);
            const ordem = ordemResult.rows[0];
            if (servicos_ids && servicos_ids.length > 0) {
                for (const servicoId of servicos_ids) {
                    await cliente.query(`INSERT INTO ordem_servicos (ordem_servico_id, servico_id, quantidade, preco_unitario)
             SELECT $1, $2, 1, preco_base FROM servicos WHERE id = $2`, [ordem.id, servicoId]);
                }
            }
            if (pecas && pecas.length > 0) {
                for (const peca of pecas) {
                    const pecaInfo = await cliente.query('SELECT preco_unitario FROM pecas WHERE id = $1', [peca.peca_id]);
                    if (pecaInfo.rows.length === 0)
                        continue;
                    await cliente.query(`INSERT INTO ordem_pecas (ordem_servico_id, peca_id, quantidade, preco_unitario)
             VALUES ($1, $2, $3, $4)`, [ordem.id, peca.peca_id, peca.quantidade, pecaInfo.rows[0].preco_unitario]);
                }
            }
            await cliente.query('COMMIT');
            await (0, emissor_de_eventos_1.emitirEvento)('ORDEM_CRIADA', {
                ordem_id: ordem.id,
                cliente_id,
                veiculo_id,
    ...
    async atualizarStatus(req, res) {
        ...
        const ordemAnterior = await (0, base_de_dados_1.query)('SELECT * FROM ordens_servico WHERE id = $1', [id]);
        if (ordemAnterior.rows.length === 0) {
            res.status(404).json({ mensagem: 'Ordem não encontrada' });
            return;
        }
        const updates = ['estado = $1', 'atualizado_em = NOW()'];
        const values = [estado];
        ...
        values.push(id);
        await (0, base_de_dados_1.query)(`UPDATE ordens_servico SET ${updates.join(', ')} WHERE id = $${values.length}`, values);
        const ordemAtualizada = (await (0, base_de_dados_1.query)('SELECT * FROM ordens_servico WHERE id = $1', [id])).rows[0];
        if (estado === 'concluida') {
            await (0, emissor_de_eventos_1.emitirEvento)('ORDEM_CONCLUIDA', {
        ...
        await redis_1.default.del(`ordem:${id}`);
        res.json(ordemAtualizada);
    }
    async atribuirTecnico(req, res) {
        const { id } = req.params;
        const { tecnico_id } = req.body;
        await (0, base_de_dados_1.query)('UPDATE ordens_servico SET tecnico_id = $1, atualizado_em = NOW() WHERE id = $2', [tecnico_id, id]);
        res.json({ mensagem: 'Técnico atribuído com sucesso' });
    }
    async registarPagamento(req, res) {
        const { id } = req.params;
        const { valor, metodo, referencia_gateway, id_transacao } = req.body;
        const result = await (0, base_de_dados_1.query)(`INSERT INTO pagamentos (ordem_servico_id, valor, metodo, referencia_gateway, id_transacao, estado)
       VALUES ($1, $2, $3, $4, $5, 'concluido')
       RETURNING *`, [id, valor, metodo, referencia_gateway, id_transacao]);
        await (0, base_de_dados_1.query)('UPDATE ordens_servico SET estado_pagamento = $1, atualizado_em = NOW() WHERE id = $2', ['pago', id]);
        await (0, emissor_de_eventos_1.emitirEvento)('PAGAMENTO_CONFIRMADO', {
            ordem_id: parseInt(id),
...
exports.default = new OrdensServico();
//# sourceMappingURL=ordens.servico.js.map