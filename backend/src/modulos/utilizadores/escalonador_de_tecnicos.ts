...
export default {
  async otimizarAlocacoes(): Promise<void> {
    ...
    const tecnicosResult = await query(
      `SELECT u.id, u.nome, COUNT(o.id) as carga
       FROM utilizadores u
    ...
    if (ordensResult.rows.length > 0) {
      await emitirEvento('AGENDAMENTO_ALTERADO', { acao: 'alocacao_automatica', total: ordensResult.rows.length });
    }
  }
};