...
export default function NotificacaoSinoAdmin() {
  const [naoLidas, setNaoLidas] = useState(0);

  useEffect(() => {
    const fetchNotificacoes = async () => {
      try {
        const res = await fetch('/api/admin/notificacoes?naoLidas=true');
  ...
  return (
    <div className="relative">
      <Link href="/modulos/admin/notificacoes" className="relative block">
