...
import { LogOut, LayoutDashboard, Users, Wrench, ClipboardList, Bell, FileText } from 'lucide-react';
import MobileMenuToggle from '@/components/MobileMenuToggle';
import SidebarCloseButton from '@/components/SidebarCloseButton';

export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  const session = await getServerSession(authOptions);
  if (!session || session.user.role !== 'administrador') redirect('/login');
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex min-h-screen">
        ...
        <aside
          id="sidebar"
          className="fixed lg:static inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 transform -translate-x-full lg:translate-x-0 transition-transform duration-300"
        >
          ...
          <div className="p-6">
            ...
            <nav className="space-y-2">
              <Link
                href="/modulos/admin/dashboard"
                className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 transition text-gray-700"
              >
                <LayoutDashboard size={20} /> Dashboard
              </Link>
              <Link
                href="/modulos/admin/documentos"
                className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 transition text-gray-700"
              >
              ...
              <Link
                href="/modulos/admin/tecnicos"
                className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 transition text-gray-700"
              >
                <Wrench size={20} /> Técnicos
              </Link>
              <Link
                href="/modulos/admin/ordens"
                className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 transition text-gray-700"
              >
        ...
        <main className="flex-1 flex flex-col min-h-screen">
          <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-sm border-b border-gray-200 p-4 flex justify-end items-center gap-4">
            <Link href="/modulos/admin/notificacoes" className="text-gray-500 hover:text-primary transition">
              <Bell size={20} />
            </Link>
          </header>
