...
exports.executarProcessador = executarProcessador;
const base_de_dados_1 = require("../configuracao/base_de_dados");
const cliente_do_agente_1 = require("../agente/cliente_do_agente");
...
async function executarAcao(acao) {
    try {
        switch (acao.tipo) {
           ...
           VALUES ($1, $2, $3, 'pendente')`, [acao.parametros.cliente_id, acao.parametros.canal || 'whatsapp', acao.parametros.mensagem]);
                break;
            case 'reagendar_ordem':
                await (0, base_de_dados_1.query)(`INSERT INTO reagendamentos_auto (ordem_servico_id, data_original, data_reagendada, motivo)
           SELECT $1, data_agendada, $2, $3 FROM ordens_servico WHERE id = $1`, [acao.parametros.ordem_id, acao.parametros.nova_data, acao.parametros.motivo]);
                await (0, base_de_dados_1.query)(`UPDATE ordens_servico SET data_agendada = $1 WHERE id = $2`, [
                    acao.parametros.nova_data,
                    acao.parametros.ordem_id,
                ]);
                break;
            case 'priorizar_ordem':
                await (0, base_de_dados_1.query)(`UPDATE ordens_servico SET prioridade = $1 WHERE id = $2`, [
                    acao.parametros.nova_prioridade,
                    acao.parametros.ordem_id,
                ]);
                break;
            case 'sugerir_tecnico':
                await (0, base_de_dados_1.query)(`UPDATE ordens_servico SET tecnico_id = $1 WHERE id = $2`, [
...
async function executarProcessador() {
    try {
        // Buscar eventos não processados com até 3 tentativas
        const result = await (0, base_de_dados_1.query)(`SELECT id, tipo, payload, tentativas
       FROM eventos
       ...
       LIMIT 10`);
        ...
        for (const row of result.rows) {
            ...
            try {
                ...
                let decisao;
                try {
                    decisao = await (0, cliente_do_agente_1.processarEventoNoAgente)({
                        id: evento.id,
                        tipo: evento.tipo,
                ...
                await (0, base_de_dados_1.query)(`UPDATE eventos SET processado = TRUE, processado_em = NOW() WHERE id = $1`, [evento.id]);
                console.log(`Evento ${evento.id} processado com sucesso`);
            }
            catch (erro) {
                console.error(`Falha no evento ${evento.id}:`, erro.message);
                await (0, base_de_dados_1.query)(`UPDATE eventos SET tentativas = tentativas + 1, ultimo_erro = $2 WHERE id = $1`, [evento.id, erro.message]);
            }
        }
    }
    catch (erro) {
...
async function aplicarRegrasDuras(tipo, payload) {
    const acoes = [];
